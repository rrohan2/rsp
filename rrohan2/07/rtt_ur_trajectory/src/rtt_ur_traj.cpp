...
rtt_ur_traj::rtt_ur_traj( const std::string& name ) :
  ...
  addOperation("GetC", &rtt_ur_traj::findCartesianPos, this, RTT::OwnThread );
...
bool rtt_ur_traj::configureHook(){
  ...
  if(status == std::future_status::ready){
    ...
    if(response->values.size()>0){
      ...
      std::cout << "description received" << std::endl;
    }
    else{
  ...
  if( kdl_parser::treeFromString( robot_description, tree)){
    RTT::log(RTT::Info) << "descrip parsed" << RTT::endlog();
  ...
  joint_nums = chain.getNrOfJoints();
  rml = new ReflexxesAPI(joint_nums , getPeriod() );
  ip_q = new RMLPositionInputParameters( joint_nums );
  op_q = new RMLPositionOutputParameters( joint_nums );
  ip_c = new RMLPositionInputParameters( joint_nums );
  op_c = new RMLPositionOutputParameters( joint_nums );
  joint_states.resize(joint_nums);
...
void rtt_ur_traj::updateHook(){
  ...
  KDL::JntArray temp_joint_states(joint_nums);
...
void rtt_ur_traj::moveC( const KDL::Frame& goal_cartes ){
  KDL::JntArray temp(joint_nums);
  temp(0) = goal_cartes.p(0);
  temp(1) = goal_cartes.p(1);
  temp(2) = goal_cartes.p(2);
  // goal_cartes.M.GetRPY(temp(3),temp(4),temp(5));
  goal_cartes.M.GetRPY(temp(3),temp(4),temp(5));
  ...
  KDL::JntArray q_sol(joint_nums);
  int retval = invkin_solver->CartToJnt(q_init, goal_cartes, q_sol);
  for(int i=0;i<joint_nums;i++){
...
KDL::Frame rtt_ur_traj::findCartesianPos(){
  KDL::Frame frame0;
  KDL::JntArray q(joint_nums);
  for(int i=0;i<joint_nums;i++){
    q(i) = ip_q->CurrentPositionVector->VecData[i];
